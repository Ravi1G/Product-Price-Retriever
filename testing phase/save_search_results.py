...
def flipkart(x):
    ...
    with open("flipkart_results.html","w") as f:
...
def amazon_india(x):
    ...
    results=urllib2.urlopen(url,data)
    with open("amazon_india_results.html","w") as f:
        f.write(results.read())
...
def amazon(x):
    ...
    data=urllib.urlencode({'url':'search-alias%3Daps','field-keywords':x1,'rh':'i%3Aaps%2Ck%3A'+x1})
    results=urllib2.urlopen(url,data)
    with open("amazon_results.html","w") as f:
        f.write(results.read())
...
def yebhi(x):
    ...
    results=urllib2.urlopen(url,data)
    with open("yebhi_results.html","w") as f:
        f.write(results.read())
...
def snapdeal(x):
    ...
    results=urllib2.urlopen(url,data)
    with open("snapdeal_results.html","w") as f:
        f.write(results.read())
...
def ebay(x):
    ...
    data=urllib.urlencode({'_trksid':'p2050601.m570.l1311.R1.TR3.TRC0.A0.X'+x1,'_nkw':x1,'_sacat':'0','_from':'R40'})
    results=urllib2.urlopen(url,data)
    with open("ebay_results.html","w") as f:
        f.write(results.read())
...
def view_in_browser(x):
    webbrowser.open("flipkart_results.html")
    webbrowser.open("amazon_results.html")
    webbrowser.open("amazon_india_results.html")
    webbrowser.open("yebhi_results.html")
    webbrowser.open("snapdeal_results.html")
    webbrowser.open("ebay_results.html")
